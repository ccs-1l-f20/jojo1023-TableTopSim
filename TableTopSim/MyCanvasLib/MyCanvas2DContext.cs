...
namespace MyCanvasLib
{
    ...
    public class MyCanvas2DContext : MyRenderingContext
    {
        #region Constants
        ...
        private const string GET_IMAGE_DATA = "getImageData";
        #endregion
        ...
        internal MyCanvas2DContext(MyCanvasClass reference) : base(reference, CONTEXT_NAME)
        {
        }
        ...
        public async Task<byte[]> GetImageDataAsync(int sx, int sy, int sw, int sh)
        {
            JsonElement js = (JsonElement)(await this.CallMethodAsync<object>(GET_IMAGE_DATA, sx, sy, sw, sh));

            js.TryGetProperty("data", out js);
            byte[] bData = new byte[4];
            for (int i = 0; i < bData.Length; i++)
            {
                JsonElement current;
                js.TryGetProperty(i.ToString(), out current);
                byte val;
                current.TryGetByte(out val);
                bData[i] = val;
            }
            return bData;
        }
